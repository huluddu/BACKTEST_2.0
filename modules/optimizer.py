...
class SearchSpace:
    ...
    offset_trend_short_choices: list = field(default_factory=lambda: ["1"])
    offset_trend_long_choices:  list = field(default_factory=lambda: ["1"])
...
def make_full_search_space(ma_choices=None, use_trend=True, use_atr=True,
    ...
    return SearchSpace(
        ...
        offset_trend_short_choices=_OFFSET_FULL,
        offset_trend_long_choices=_OFFSET_FULL,
...
def _build_params_from_trial(trial, ss, base_params, disable_tp=False):
    ...
    p.offset_trend_short = _si(trial.suggest_categorical("off_ts",      ss.offset_trend_short_choices))
    p.offset_trend_long  = _si(trial.suggest_categorical("off_tl",      ss.offset_trend_long_choices))
...
def _params_from_dict(tp, base_params):
    ...
    p.offset_trend_short = _si(tp.get("off_ts", "1"))
    p.offset_trend_long  = _si(tp.get("off_tl", "1"))
...
def run_optimization(
    ...
    for t in trials:
        ...
        rows.append({
            ...
            "offset_trend_short":  _si(tp.get("off_ts", "1")),
            "offset_trend_long":   _si(tp.get("off_tl", "1")),
