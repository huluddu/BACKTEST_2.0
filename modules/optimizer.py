"""
optimizer.py - Optuna 베이지안 최적화 (Train/Test 없음, bool→문자열 변환)
"""
...
# bool 헬퍼 (Optuna suggest_categorical에 bool 넣으면 numpy 오류 발생)
...
def _build_params_from_trial(trial, ss, base_params, disable_tp=False):
    ...
    p.ma_buy         = int(trial.suggest_categorical("ma_buy",      ss.ma_buy_choices))
    p.ma_sell        = int(trial.suggest_categorical("ma_sell",     ss.ma_sell_choices))
    p.offset_cl_buy  = int(trial.suggest_categorical("off_cl_buy",  ss.offset_cl_buy_choices))
    p.offset_ma_buy  = int(trial.suggest_categorical("off_ma_buy",  ss.offset_ma_buy_choices))
    p.offset_cl_sell = int(trial.suggest_categorical("off_cl_sell", ss.offset_cl_sell_choices))
    p.offset_ma_sell = int(trial.suggest_categorical("off_ma_sell", ss.offset_ma_sell_choices))
    ...
    p.ma_trend_short = int(trial.suggest_categorical("ma_ts",       ss.ma_trend_short_choices))
    p.ma_trend_long  = int(trial.suggest_categorical("ma_tl",       ss.ma_trend_long_choices))
    p.stop_loss_pct  = float(trial.suggest_categorical("sl",        ss.stop_loss_choices))
    p.use_atr_stop   = _s2b(trial.suggest_categorical("use_atr",   ss.use_atr_stop_choices))
    p.atr_multiplier = float(trial.suggest_categorical("atr_mult",  ss.atr_mult_choices))
    p.take_profit_pct = 0.0 if disable_tp else float(trial.suggest_categorical("tp", ss.take_profit_choices))

    p.use_rsi_filter = _s2b(trial.suggest_categorical("use_rsi", ss.use_rsi_choices))
    if p.use_rsi_filter:
        p.rsi_period = int(trial.suggest_categorical("rsi_period", ss.rsi_period_choices))
        p.rsi_max    = int(trial.suggest_categorical("rsi_max",    ss.rsi_max_choices))
        p.rsi_min    = 100 - p.rsi_max

    p.use_bollinger = _s2b(trial.suggest_categorical("use_bb", ss.use_bb_choices))
    if p.use_bollinger:
        p.bb_period = int(trial.suggest_categorical("bb_period",   ss.bb_period_choices))
        p.bb_std    = float(trial.suggest_categorical("bb_std",    ss.bb_std_choices))
...
def _params_from_dict(tp, base_params):
    ...
    p.rsi_period      = int(tp.get("rsi_period", 14))
    p.rsi_max         = int(tp.get("rsi_max", 70))
    p.use_bollinger   = _s2b(tp.get("use_bb", "0"))
    p.bb_period       = int(tp.get("bb_period", 20))
    p.bb_std          = float(tp.get("bb_std", 2.0))
